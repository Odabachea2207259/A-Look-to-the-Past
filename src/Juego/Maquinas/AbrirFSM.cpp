...
namespace IVJ
{
	...
	FSM* AbrirFSM::onInputs(const CE::IControl& control)
	{
		if(termino)
			return new IdleFSM();
	...
	void AbrirFSM::onEntrar(const Entidad& obj)
	{
		...
		vectores = cargarSprites(j,"Abrir");
	...
	void AbrirFSM::onSalir(const Entidad& obj){
        auto c = obj.getComponente<CE::IControl>();

		c->abrir = false;
	}  

	void AbrirFSM::onUpdate(const Entidad& obj,float dt)
	{
		tiempo = tiempo - 1*dt;
