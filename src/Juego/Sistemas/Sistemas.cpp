...
namespace IVJ
{
	...
	std::vector<std::shared_ptr<CE::ISprite>> SistemaGetQueue(std::vector<std::shared_ptr<IVJ::Entidad>> turnos)
	{
		std::vector<std::shared_ptr<CE::ISprite>> queue;

		for(auto & ente : turnos){
			auto nombre = "cabeza" + ente->getNombre()->nombre;
			auto textura = CE::GestorAssets::Get().getTextura(nombre);
			auto size = textura.getSize();

			queue.push_back(std::make_shared<CE::ISprite>(
				textura,
				size.x,size.y,
				0.7f
			));
